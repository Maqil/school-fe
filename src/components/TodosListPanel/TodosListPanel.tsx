...
import {
  ...
  ListLoading,
  ListRowWrapper
} from "./TodosListPanel.style";
import { useEffect, useState } from "react";
import { RowSummaryAirbill } from "../ShipmentsRow/ShipmentsRow.style";

function TodosListPanel(props) {
  ...
  useEffect(() => {
    if (rows && rows.length !== 0) {
      setTodoIndex(rows.map(i => i.todo.todoAiid))
    }
  }, [rows]);
        ...
        >
          {rows.length !==0 &&
           stableSort(rows, getComparator(listState.sortBy)).map(row => 
            // {
            //     console.log("erer", row);
            //   }
            (
            <ShipmentsRow
              key={Object.values(rows).map((i:any) => i.todoAiid)}
              row={row.todo}
              rowId={`row-${todoIndex}`}
            /> 
            )
